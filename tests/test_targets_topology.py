...
def test_rc_python_facade_publish_state_is_explicit() -> None:
    ...
    assert "PyPI package is published at v0.3.3" in blockers["core"]
...
def test_current_pypi_manual_actions_track_resolved_publishers() -> None:
    ...
    expected = {
        "pypi-publisher-core": ("nirs4all-core", "v0.3.3"),
...
def test_current_runiverse_manual_actions_cover_stale_rc_rebuilds() -> None:
    actions = {action.id: action for action in load_actions(ROOT / "ops" / "manual-actions.yaml")}
    expected = {
        "runiverse-core-rebuild": ("nirs4all-core", "nirs4all", "v0.3.3", "todo"),
    ...
    snapshot = Snapshot(
        ...
        packages=[
            PackageStatus(
                id="nirs4all-core",
                repo="nirs4all-core",
                source=PackageSource(expected_prod_version="0.3.3"),
                rollup="green",
                targets=[
                    TargetStatus(
                        registry="r-universe",
